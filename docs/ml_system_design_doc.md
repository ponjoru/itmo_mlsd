...
Архитектура: \
![Arch](../assets/mlsd_arch.png)

Диаграмма потоков данных: \
![Runtime Flow](../assets/mlsd_runtime_flow.png)

**Компоненты и их предназначение**
- Orchestrator (управление камерами)
   - Создаёт/останавливает worker’ы по списку камер
   - Следит за health (FPS, лаг, reconnect).
   - Управляет конфигами (параметры детектора, зоны, трешхолды).
- Worker (на 1 камеру)
   - Thread 1: Decoder
      - RTSP reconnect/backoff
      - декодирование кадров
      - push в bounded queue
   - Thread 2: Manager
      - владеет экземпляром C++ библиотеки
      - вытаскивает кадры из очереди
      - вызывает `process(frame)`
      - приводит события к модели БД
      - пишет в БД
- C++ ML Library
   - Детекция/треккинг/ивенты
   - Выдаёт события: `START` (с event_id), `END` (тот же event_id) `metadata` (bbox, confidence, zone_id, track_id, snapshots, etc.)
- Events DB
   - Хранит события и их статус/метаданные
   - обеспечивает запросы UI
- Local UI (Frontend/Backend)
   - Backend читает из DB (polling или subscribe)
   - Frontend показывает ленту событий + детали

**C++ Lib API:**
```
process(frame: Frame, ts: long, camera_id: string) -> list<Event>
```
...
Инфраструктура - сервера заказчика с Intel процессорами с AMX инструкциями

Плюсы:
- Не нужно докупать новое оборудование
- Дешево в содержании
- Можно использовать прямо сейчас

Минусы:
- Вычислительные мощности ограничены, не можем использовать SOTA модели и подходы

Альтернативы:
- Облако и наши сервера (не подходит из-за вопросов конфиденциальности)
- Более продвинутые сервера у заказчика (нужно обосновать необходимость, заказчику нужно вкладывать деньги, время на установку)

Текущий выбор оптимален с точки зрения времени начала использования и стоимости

### 4.3. Требования к работе системы
- Доступность: 99.5%
- Стабильность обработки: <1 аварийного сбоя в сутки
- Потеря кадров видеопотока: не более 5%
- Пропускная способность: 100 потоков 1080p 25fps
- Задержка: <5 секунд на сработку

### 4.4. Способы масштабирования
- Расширение до 100 камер - покрывается текущим железом. Создаем дополнительный воркер с 2 тредами на каждую новую камеру
- более 100 камер - требуется докупать дополнительные вычислительные ноды и формировать из сервера кластер
